...
function TextInput({ label, name, className }: { label: string; name: string, className?: string }) {
	const [searchParams] = useSearchParams()
...
function FormGroup({
	...
	return (
		<div className="flex flex-col [&:not(:first-child)]:ml-4">
	...
	return (
		<>
			...
			>
				...
				<TextInput label="Search" name="name" className="items-end ml-auto" />
				...
				{weapons.map((weapon) => {
					return (
						<li
							...
							key={weapon.id}
						>
							<div className="pointer-events-none absolute top-0 right-0 aspect-video h-full overflow-hidden z-0 ">
								<img
									alt=""
							...
							<Link to={weapon.slug} className="block h-full w-full p-4 z-10 isolate">
			...
			{weapons.length < 1 ? (
				...
				<div className="px-4 py-6 sm:px-0">
					<div className="h-96 rounded-lg border-4 border-dashed border-gray-200 grid place-content-center">
						<span className="font-heading font-black text-lg text-neutral-400">No results</span>
					</div>
				</div>
			) : null}
		</>
